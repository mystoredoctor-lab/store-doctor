...
export function HealthScoreGauge({ score, size = "md", showLabel = true, className }: HealthScoreGaugeProps) {
  ...
  return (
    <div className={cn("flex flex-col items-center gap-2", className)} data-testid="health-score-gauge">
      <div className="relative" style={{ width: config.width, height: config.width }}>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <span className={cn(config.fontSize, "font-bold", getScoreColor(score))} data-testid="health-score-value">{score}</span>
        </div>
      </div>
